...
export function CalendarMonthView({ month, selectedDate, markedDates, onMonthChange, onSelectDate, touchOptimized = false }: CalendarMonthViewProps) {
  ...
  return (
    <div className={cn("panel-strong rounded-2xl border border-slate-200 bg-white p-4 shadow-card", touchOptimized && "p-4 lg:p-5 xl:p-6")}>
      <div className="mb-3 flex items-center justify-between">
        <Button type="button" variant="secondary" onClick={() => onMonthChange(addMonths(month, -1))} className={touchOptimized ? "h-11 min-w-11 text-lg lg:h-12 lg:min-w-12 lg:text-xl" : ""}>
          ←
        </Button>
        <h2 className={cn("font-semibold text-slate-900", touchOptimized ? "text-2xl lg:text-3xl" : "text-xl")}>{formatMonthGerman(month)}</h2>
        <Button type="button" variant="secondary" onClick={() => onMonthChange(addMonths(month, 1))} disabled={!canGoForward} className={touchOptimized ? "h-11 min-w-11 text-lg lg:h-12 lg:min-w-12 lg:text-xl" : ""}>
          →
        </Button>
      </div>

      <div className={cn("mb-2 grid grid-cols-7 gap-2 text-center font-semibold text-slate-500", touchOptimized ? "text-sm lg:text-base" : "text-sm")}>
        {weekdays.map((day) => (
          ...
          return (
            <button
              ...
              className={cn(
                ...
                touchOptimized && "h-14 rounded-xl text-lg font-semibold xl:h-16 xl:text-xl",
      ...
      <div className={cn("mt-3 flex items-center gap-2 text-slate-500", touchOptimized ? "text-base" : "text-sm")}>
