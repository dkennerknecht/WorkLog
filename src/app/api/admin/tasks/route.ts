...
export async function GET(request: NextRequest) {
  ...
  const tasks = await prisma.task.findMany({ orderBy: [{ sortOrder: "asc" }, { name: "asc" }] });
...
export async function POST(request: NextRequest) {
  ...
  try {
    const created = await createMasterRecord(prisma, "task", parsed.data.name);
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    await reorderMasterRecords(prisma, "task", parsed.data.orderedIds);
    return ok({ ok: true });
  } catch (error) {
